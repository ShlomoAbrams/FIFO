...
# 4. Compile RTL (Verilog files) with Code Coverage enabled (-cover bcesxf)
vlog -timescale "1ns/1ps" -reportprogress 300 -cover bcesxf \
    ../rtl/verilog/fifo_mem.v ../rtl/verilog/fifo_r_ptr.v ../rtl/verilog/fifo_w_ptr.v ../rtl/verilog/fifo_synchronizer.v ../rtl/verilog/fifo.v

# 5. Compile UVM Testbench Package WITHOUT code coverage (excludes testbench internals from DUT coverage metrics)
vlog -timescale "1ns/1ps" -sv -mfcu $UVM_INC -L mtiUvm ../uvm/fifo_pkg.sv

# 6. Compile SV Interface and Assertions WITH coverage
vlog -timescale "1ns/1ps" -sv -mfcu $UVM_INC -cover bcesxf -L mtiUvm \
	../uvm/fifo_if.sv \
	../uvm/fifo_sva.sv

# 7. Compile SV Top Module WITHOUT coverage (keeps testbench initial blocks out of DUT metrics)
vlog -timescale "1ns/1ps" -sv -mfcu $UVM_INC -L mtiUvm ../uvm/fifo_top.sv
...
add wave -group "Clocks & Resets" -color "White" -label "wclk" sim:/fifo_top/dut/wclk
add wave -group "Clocks & Resets" -color "White" -label "wrst_n" sim:/fifo_top/dut/wrst_n
add wave -group "Clocks & Resets" -color "White" -label "rclk" sim:/fifo_top/dut/rclk
add wave -group "Clocks & Resets" -color "White" -label "rrst_n" sim:/fifo_top/dut/rrst_n

# Write Domain (Primary - Always Visible)
add wave -group "Write Domain" -color "Cyan" -radix hex -label "wdata" sim:/fifo_top/dut/fifo_mem_unit/wdata
add wave -group "Write Domain" -color "Cyan" -radix unsigned -label "waddr" sim:/fifo_top/dut/fifo_mem_unit/waddr
add wave -group "Write Domain" -color "Cyan" -label "wclken" sim:/fifo_top/dut/fifo_mem_unit/wclken
add wave -group "Write Domain" -color "White" -label "wclk" sim:/fifo_top/dut/wclk

# Read Domain (Primary - Always Visible)
add wave -group "Read Domain" -color "SpringGreen" -radix hex -label "rdata" sim:/fifo_top/dut/fifo_mem_unit/rdata
add wave -group "Read Domain" -color "SpringGreen" -radix unsigned -label "raddr" sim:/fifo_top/dut/fifo_mem_unit/raddr
add wave -group "Read Domain" -color "SpringGreen" -label "rclken" sim:/fifo_top/dut/fifo_mem_unit/rclken
add wave -group "Read Domain" -color "White" -label "rclk" sim:/fifo_top/dut/rclk
...
add wave -group "FIFO Full Condition" -color "Red" -label "wfull" sim:/fifo_top/dut/wfull
...
add wave -group "FIFO Empty Condition" -color "Red" -label "rempty" sim:/fifo_top/dut/rempty
add wave -group "FIFO Empty Condition" -color "White" -label "rclk"  sim:/fifo_top/dut/rclk

# READ-TO-WRITE SYNCHRONIZATION (CDC)
add wave -group "Read-to-Write Sync" -color "White" -label "rclk" sim:/fifo_top/dut/rclk
add wave -group "Read-to-Write Sync" -color "Magenta" -label "ptr_g" sim:/fifo_top/dut/read_to_write_sync/ptr_g
add wave -group "Read-to-Write Sync" -color "White" -label "wclk" sim:/fifo_top/dut/wclk
add wave -group "Read-to-Write Sync" -color "Magenta" -label "q1ptr_g (Stage 1)" sim:/fifo_top/dut/read_to_write_sync/q1ptr_g
...
add wave -group "Write-to-Read Sync" -color "White" -label "wclk" sim:/fifo_top/dut/wclk
add wave -group "Write-to-Read Sync" -color "Pink" -label "ptr_g" sim:/fifo_top/dut/write_to_read_sync/ptr_g
add wave -group "Write-to-Read Sync" -color "White" -label "rclk" sim:/fifo_top/dut/rclk
add wave -group "Write-to-Read Sync" -color "Pink" -label "q1ptr_g (Stage 1)" sim:/fifo_top/dut/write_to_read_sync/q1ptr_g
add wave -group "Write-to-Read Sync" -color "Pink" -label "q2ptr_g (Stage 2)" sim:/fifo_top/dut/write_to_read_sync/q2ptr_g
