class fifo_w_monitor #(parameter DATA_WIDTH = 8) extends uvm_monitor; // BLUEPRINT: Defines Write Monitor based on template
	`uvm_component_param_utils(fifo_w_monitor#(DATA_WIDTH)) // FACTORY: Register the Monitor in UVM library so it can be used later
