class fifo_r_agent extends uvm_agent; // Defines "Read Agent" to group Read Sequencer, Driver, and Monitor
	`uvm_component_utils(fifo_r_agent) // FACTORY: Register the agent in UVM library 
	
    fifo_r_driver r_driver; // read driver handle
    fifo_r_monitor r_monitor; // read monitor handle
    uvm_sequencer #(fifo_transaction) r_sequencer; // read sequencer handle
	...
	virtual function void build_phase(uvm_phase phase); // BUILD PHASE: Runs at Time 0 to fetch configuration data before simulation starts
        ...
        r_monitor = fifo_r_monitor::type_id::create("r_monitor", this); // Create Read Monitor
        if(get_is_active() == UVM_ACTIVE) begin
            r_driver = fifo_r_driver::type_id::create("r_driver",this); // Create Read Driver
            r_sequencer = uvm_sequencer#(fifo_transaction)::type_id::create("r_sequencer",this); // Create Read Sequencer
		end
	endfunction
