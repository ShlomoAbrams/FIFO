...
class fifo_w_driver extends uvm_driver #(fifo_transaction); // Defines "Write Worker" based on UVM Driver template that handles "fifo transaction" packets. 
	`uvm_component_utils(fifo_w_driver) // FACTORY: Register the driver in UVM library so it can be used later
	...
	vif.w_d_cb.winc <= 1'b0; // Initialize to 0, so we dont write garbage to fifo in startup
	vif.w_d_cb.wdata <= 8'h00; 
	wait(vif.wrst_n === 1'b1); // wait until reset to turn off to drive the data
	`uvm_info("DRV", "Reset released, starting to drive data", UVM_LOW) 
	forever begin 
		seq_item_port.get_next_item(req);	// HANDSHAKE: Ask sequencer (boss) for next peice of data to process
	...
	virtual task drive_item(fifo_transaction tr); // TRANSLATOR: Task that turn software into hardware reality.
		repeat (tr.delay) @(vif.w_d_cb); // STRESS TESTING: wait for randomized delay to simulate slow or fast traffic
		...
		if (tr.en) begin // if the transaction says "Write" drive winc & wdata
			...
			vif.w_d_cb.winc <= 1'b0; 	// Disable too prevent double write
		end
	endtask
endclass